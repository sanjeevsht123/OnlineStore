...
class _CartState extends State<Cart> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
                        ...
                        return Dismissible(
                          ...
                          child: ListTile(
                            leading: CircleAvatar(
                              backgroundImage:
                                  NetworkImage(index_num.product.image),
                            ),
                            title: Text(index_num.product.title),
                            subtitle: Text(
                                "\$${index_num.product.price.toString()} X ${index_num.quantity} = \$${index_num.product.price * index_num.quantity} "),
                            trailing: IconButton(
                              onPressed: () {
                                showDialog(
                                    context: context,
      ...
      bottomSheet: widget.cart.length == 0
          ? SizedBox()
          : Container(
